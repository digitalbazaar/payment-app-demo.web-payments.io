/*
 * payment-app-demo.payment-app-demo.web-payments.io default configuration.
 *
 * Copyright (c) 2016 Digital Bazaar, Inc. All rights reserved.
 */
var config = require('bedrock').config;
var path = require('path');

// location of logs
var _logdir = '/tmp/payment-app-demo.web-payments.io';

// core
// 0 means use # of cpus
config.core.workers = 1;
config.core.master.title = 'payment-appd';
config.core.worker.title = 'payment-appd-worker';
config.core.worker.restart = false;

// logging
config.loggers.app.filename =
  path.join(_logdir, 'payment-app-demo.web-payments.dev-app.log');
config.loggers.access.filename =
  path.join(_logdir, 'payment-app-demo.web-payments.dev-access.log');
config.loggers.error.filename =
  path.join(_logdir, 'payment-app-demo.web-payments.dev-error.log');
config.loggers.email.silent = true;
config.loggers.email.to = ['cluster@payment-app-demo.web-payments.io'];
config.loggers.email.from = 'cluster@payment-app-demo.web-payments.io';

// server info
config.server.port = 53443;
config.server.httpPort = 53080;
config.server.bindAddr = ['payment-app-demo.web-payments.dev'];
config.server.domain = 'payment-app-demo.web-payments.dev';
config.server.host = 'payment-app-demo.web-payments.dev:53443';
config.server.baseUri = 'https://' + config.server.host;

// express info
config.express.session.secret = 'NOTASECRET';
config.express.session.key = 'payment-app-demo.web-payments.io.sid';
config.express.session.prefix = 'payment-app-demo.web-payments.io.';

// views
// branding
config.views.brand.name = 'payment-app-demo.web-payments.io Development';

// update view vars
config.views.vars.baseUri = config.server.baseUri;
config.views.vars.title = config.views.brand.name;
config.views.vars.siteTitle = config.views.brand.name;
config.views.vars.supportDomain = config.server.domain;
config.views.vars.debug = false;
config.views.vars.footer.show = false;
// FIXME: add logo img
config.views.vars.style.brand.alt = config.views.brand.name;
config.views.vars.style.brand.src = null;//'/img/brand.png';
//config.website.views.vars.style.brand.height = '24';
//config.website.views.vars.style.brand.width = '25';
// contact info
config.views.vars.contact.address = {
  label: 'Digital Bazaar, Inc.',
  address:
    '123 FIXME\n' +
    'FIXME, XX 12345\n' +
    'United States of America',
  htmlAddress:
    '123 FIXME<br/>' +
    'FIXME, XX 12345<br/>' +
    'United States of America'
};
config.views.vars.demoWarningUrl = null;

// payment-app-demo.web-payments.io config
config.views.vars['payment-app-demo.web-payments.io'] = {};
